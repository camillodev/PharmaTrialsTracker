...
- Average Symptom Severity: ${data.avgSymptomSeverity.toFixed(1)} (scale 1-10)
- Number of Outliers: ${data.outlierCount}
- Outlier Percentage: ${((data.outlierCount / data.totalPatients) * 100).toFixed(1)}%

Recent Outlier Events (for trend analysis):
${data.recentOutliers.map((event, i) => `${i + 1}. ${event}`).join('\n')}
